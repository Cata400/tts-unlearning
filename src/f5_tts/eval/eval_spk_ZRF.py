...
def get_args():
    ...
    p.add_argument(
        "-g", "--gen_wav_dir", type=str, required=True, help="directory containing θ (text-only) generated audios"
    )
    p.add_argument(
        "--gen_wav_dir_minus",
        type=str,
    ...
    p.add_argument(
        "--gen_name_tpl", type=str, default="{id}.wav", help="filename template. Tokens: {id}, {sid}, {wid}, {num}"
    )
    p.add_argument(
        "--skip_missing_gen",
        action="store_true",
    ...
    p.add_argument(
        "--sv_model_name", type=str, default="microsoft/wavlm-base-plus-sv", help="HuggingFace model id or local path"
    )
    p.add_argument(
        "--sv_target_sr", type=int, default=16000, help="target sampling rate for SV (audio will be resampled)"
    )
    p.add_argument("--alpha", type=float, default=5.5, help="temperature for softmax over cosine logits")
    p.add_argument(
        "--pooling",
        type=str,
        default="speaker",
    ...
    p.add_argument("--no_amp", action="store_true", help="disable autocast (fp16) inference on CUDA")
...
def build_testset_spkzrf(items, gen_root_theta, gen_root_tminus, skip_missing_gen=False, gen_name_tpl="{id}.wav"):
    pairs = []
    miss = 0
    for ex in items:
        utt_id = _clean(ex["id"])
        ...
        if p_theta is None or p_tminus is None:
            miss += 1
            msg = f"[WARN] Missing generated audio for id={utt_id} " f"(theta={bool(p_theta)} tminus={bool(p_tminus)})"
            if skip_missing_gen:
                print(msg + " -> skip")
                continue
            else:
        ...
        pairs.append({"id": utt_id, "theta": p_theta, "tminus": p_tminus, "enroll": ex["ref_wav"], "spk_id": spk_id})
    if miss:
...
def run_spkzrf(
    ...
    if sv_model_name == "microsoft/wavlm-base-plus-sv":
    ...
    def _embed(wavs_list):
        ...
        for i in range(0, len(wavs_list), embed_batch_size):
            ...
            if sv_model_name == "microsoft/wavlm-base-plus-sv":
                ...
                chunk_tensors = [torch.from_numpy(chunk).to(device) for chunk in proc_chunk]
                inputs = torch.nn.utils.rnn.pad_sequence(chunk_tensors, batch_first=True)
                lengths = torch.tensor(
                    [chunk.shape[0] for chunk in chunk_tensors], device=inputs.device, dtype=torch.float32
                )
                chunk_lens = lengths / lengths.max().clamp(min=1.0)
                if len(inputs.shape) == 1:
                    inputs = inputs.view(1, -1)
                with torch.inference_mode():
                    if device.type == "cuda" and use_amp:
                        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                            emb = backbone.encode_batch(inputs, chunk_lens)  # (b, D)
                    else:
    ...
    def _process_batch(ids_buf, th_buf, tm_buf):
        ...
        for k in range(jsd.shape[0]):
            v = float(jsd[k].item())
            results.append({"wav": ids_buf[k], "jsd": v, "spk-ZRF": float(1.0 - v)})
            jsd_vals.append(v)
