...
def build_finetune_strategy(model_cfg_dict: dict) -> FineTuningStrategy:
    ...
    At most one parameter-freezing strategy (diffit XOR dit_blocks_mlp), at most one
    selector strategy (fim), and at most one SVD strategy (svdiff XOR svdiff_uv) may be
    enabled. When more than one category is enabled they are composed in the order
    `[freezer, selector, svd]` inside a `CompositeStrategy`; FIM's `apply()` runs after
    the freezer (so its Fisher profiling respects the freezer's mask) and before the
    SVD strategy (so SVD parametrization is only registered on FIM-selected tensors).
    """
