# continual (sequential) unlearning training script: one forget speaker per step.
...
def main(model_cfg):
    ...
    assert "continual" in model_cfg, (
        "The continual trainer needs a `continual` section in the config; "
        "use configs/F5TTS_v1_Base_unlearn_continual.yaml as a starting point."
    )
    continual_params = OmegaConf.to_container(model_cfg.continual, resolve=True)
    print(f"Continual unlearning params: {continual_params}")

    forget_speakers = list(model_cfg.unlearn.forget_speakers)
    assert len(forget_speakers) > 0, "unlearn.forget_speakers must list at least one speaker"
    print(f"Continual unlearning order: {forget_speakers}")
    ...
    # init trainer
    trainer = TrainerUnlearnContinual(
        model,
        teacher,
